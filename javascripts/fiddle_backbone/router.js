...
define([
	...
	'views/DBTypesList'
], function($, _, Backbone, dbTypes, schemaDef, query, myFiddleHistory, dbTypesListView ){
				...
				if (resp["short_code"])
				{
					...
					if (selectedDBType.get("context") == "browser")
					{
						if (
								...
								browserEngines["websql"].nativeSQLite
							)
						{
						...
						browserEngines[selectedDBType.get("className")].buildSchema({
							...
							success: function () {
								...
								browserEngines[selectedDBType.get("className")].getSchemaStructure({
										callback: function (schemaStruct) {
											...
											if (resp["sql"])
											{
												browserEngines[selectedDBType.get("className")].executeQuery({
													sql: resp["sql"],
													statement_separator: resp["query_statement_separator"],
