$(function () {
	...
	window.DBType = Backbone.Model.extend({
		defaults: {
			"sample_fragment":"",
			"notes":"",
			"simple_name": "",
	...
	window.DBTypesList = Backbone.Collection.extend({
		model: DBType,
	...
	window.SchemaDef = Backbone.Model.extend({
	...
	window.dbTypes = new window.DBTypesList();
	window.schemaDef = new window.SchemaDef();
	window.query = new window.Query({
		"schemaDef": window.schemaDef
	});
	
	dbTypes.on("reset", function () {
		if (this.length && !schemaDef.has('dbType'))
			schemaDef.set({ dbType: this.at(1) });
	});
